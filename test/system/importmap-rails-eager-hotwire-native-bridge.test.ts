...
const project = setupProject("importmap-rails-eager-hotwire-native-bridge")

describe("System", () => {
  test("importmap-rails-eager-hotwire-native-bridge", async () => {
    ...
    expect(project.registeredControllers.map(controller => [controller.identifier, controller.loadMode])).toEqual(
      [
        ["hello", "stimulus-loading-eager"],
        ["bridge--button", "stimulus-loading-eager"]
      ]
    )
