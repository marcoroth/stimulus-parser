...
import type { ControllersIndexFile } from "../controllers_index_file"
import type { ControllerLoadMode } from "../types"

export class ControllersIndexAnalyzer {
  private readonly controllersIndexFile: ControllersIndexFile
  private readonly ast: AST<ParserOptions>

  constructor(controllersIndexFile: ControllersIndexFile, ast: AST<ParserOptions>) {
    this.controllersIndexFile = controllersIndexFile
    this.ast = ast
  }

  get project() {
    return this.controllersIndexFile.project
  }

  get sourceFile() {
    return this.controllersIndexFile.sourceFile
  }
    ...
    walk(this.ast, {
      CallExpression: node => {
        ...
        if (callee.type === "MemberExpression" && callee.object.type === "Identifier" && callee.property.type === "Identifier") {
          ...
          if (objectName !== this.controllersIndexFile.applicationImport?.localName) return
          ...
          this.controllersIndexFile.registeredControllers.push(new RegisteredController(identifier, controller, "register"))
        }
      }
    })
  }
        ...
        if (node.callee.type === "MemberExpression" && node.callee.object.type === "Identifier" && node.callee.property.type === "Identifier") {
          ...
          if (node.callee.object.name === "require" && node.callee.property.name === "context") {
            ...
            if (pattern instanceof RegExp) {
              controllersGlob = path.join(controllerRoot, `**/*${pattern.source.replaceAll("$", "").replaceAll("\\.", ".")}`)
            }
          }
        }
      }
  ...
  private async evaluateControllerGlob(controllerRoot: string, controllersGlob: string, type: ControllerLoadMode) {
    ...
    controllerDefinitions.forEach(controller => {
      ...
      this.controllersIndexFile.registeredControllers.push(registeredController)
    })
  }
}
